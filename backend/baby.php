...
// 获取 baby 信息
function getBaby(string $id)
{
  global $conn;
  $sql =  "select id,mid,m_name,m_relate,name,gender,birthday,survey_time,premature,create_time,update_time from baby where id='$id'";
  $result = $conn->query($sql);
  if ($conn->affected_rows != 0) {
...
// 获取  member 下的 baby列表
function listBaby(string $mid)
{
  global $conn;
  $data = [];
  $sql = "select id,mid,m_name,m_relate,name,gender,birthday,survey_time,premature,create_time,update_time from baby where mid='$mid'";
