...
class Mysql
{
    ...
    private $is_error = true; //发现错误是否立即终止,默认true,建议不启用，因为当有问题时用户什么也看不到是很苦恼的
    ...
    */
    ...
    {
        // echo "dbdbdbd";
        // echo $table,$columnName,$value;
        // var_dump("INSERT INTO $table ($columnName) VALUES ($value)");
    ...
    public function update($table, $mod_content, $condition, $url = '')
    {
        ...
        // echo 'update';
        // var_dump("UPDATE $table SET $mod_content WHERE $condition");
    ...
    public function num_fields($table_name)
    {
        ...
        echo '<pre>';
        for ($i = 0; $i < $total; ++$i) {
            print_r(mysqli_fetch_field($this->result));
        }
        echo '</pre>';
        echo '<br />';
    }
    ...
    public function mysql_server($num = '')
    {
        switch ($num) {
            ...
            case 2:
                return mysql_get_host_info($this->conn); //取得 MySQL 主机信息
                break;

            case 3:
                return mysql_get_client_info($this->conn); //取得 MySQL 客户端信息
                break;

            case 4:
                return mysql_get_proto_info($this->conn); //取得 MySQL 协议信息
                break;

            default:
                return mysql_get_client_info($this->conn); //默认取得mysql版本信息
        }
    }
